...
const corsHeaders = {
  'Access-Control-Allow-Origin': '*', // Para teste. Mude para https://sisjusgestao.com.br em produção
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
  'Access-Control-Allow-Methods': 'POST, OPTIONS', // Crucial ter POST e OPTIONS
};
...
serve(async (req: Request) => {
  ...
  if (req.method === 'OPTIONS') {
    ...
    return new Response('ok preflight', { headers: corsHeaders }); // status 200 ou 204
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Erro interno na função de teste mínimo.", details: e.message }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
