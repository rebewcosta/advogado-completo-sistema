import { supabase } from '../../integrations/supabase' // CORREÇÃO DEFINITIVA AQUI
...
export const saveCliente = async (cliente: Cliente) => {
  ...
  if (cliente.id) {
    const { data, error } = await supabase.from('clientes').update(preparedCliente).eq('id', cliente.id).select().single()
    if (error) throw error
    return data
  } else {
    const { data, error } = await supabase.from('clientes').insert(preparedCliente).select().single()
    if (error) throw error
    return data
  }
}
